...
class MyApp(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def SignUrl(self):
        try:
            ...
            if len(random) == 0:
                random = "qwertyuio"
            isWGSign = self.radio_signType.isChecked()
            if isWGSign:
                gatewaySign = self.doWGSign(random, timestamp, random)
                Param.append("gatewaySign=" + gatewaySign)
                Param.append("signToken=" + random)
            random, signStr, str1, timestamp, unSignStr = self.method_sign(Param, random, timestamp)
    ...
    def doWGSign(self, random1, timestamp, random2):
        paramDict = ["signToken=" + random1]
        return self.method_sign(paramDict, random2, timestamp)[1]

    def method_sign(self, Param, random, timestamp):
        Param.sort()
        str1 = "&".join(Param)
