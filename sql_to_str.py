...
def main():
    ...
    root.minsize(920, 635)
    root.maxsize(920, 635)  # 这里主要是控制窗口的大小，让窗口大小不能改变
    # root.geometry("1366x250")
    root.title('SQL转换工具')  # 设置主窗口的标题
    # display the quit button
    text = edit(root)  #
    l = tk.Label(root, text='SQL转换工具', fg='white', bg='black', width=100)
...
def edit(root):
    edit = tk.Text(root, fg='black', bg='#DCDCDC', font='微软雅黑', width=30, height=15, )
...
def button(root, text):
    clu = 0
    transformation = tk.Button(root, text='格式化sql', fg='black', command=lambda: Transformation(text))
    getSQLField = tk.Button(root, text='获取sql字段', fg='black', command=lambda: GetSQLField(text))
    sqlFieldToBean = tk.Button(root, text='sql字段转javaBean', fg='black', command=lambda: SqlFieldToBean(text))
    md5do = tk.Button(root, text='-Md5  Creator-', fg='black', command=lambda: md5create(text))
    auther = tk.Button(root, text='冯书浩', font='微软雅黑')
...
def set0perator(o):
    ...
    return  dict_oper.get(o[1]) + (o[0][0].upper()+o[0][1:]) + "("+oper.get(o[1]) + " %(value1)s ){ this.%(value2)s = %(value3)s ; }" %value
